...
const columns = [
  ...
  columnHelper.accessor('content_type', {
    header: 'Type',
    cell: (info) => <span className="text-xs px-2 py-0.5 rounded-full bg-muted">{info.getValue()}</span>,
  }),
  ...
  columnHelper.accessor('created_at', {
    header: 'Date',
    cell: (info) => { const val = info.getValue(); return val ? new Date(val).toLocaleDateString() : '' },
  }),
]
...
export default function Library() {
  ...
  useEffect(() => {
    ...
    api.listContent(params).then((r) => { setData(r.items); setTotal(r.total) })
  ...
  const selectedItem = useMemo(() => data.find((d) => d.id === selectedId), [data, selectedId])

  return (
    <div className="flex h-full gap-4">
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex gap-2 mb-4 flex-wrap">
          <input type="text" placeholder="Search library..." value={globalFilter}
        ...
        <div className="border border-border rounded-lg overflow-auto flex-1">
          ...
          {data.length === 0 && <p className="text-center text-muted-foreground py-8 text-sm">No content found.</p>}
        </div>
      ...
      {selectedItem && (
        <div className="w-80 border border-border rounded-lg p-4 overflow-auto flex-shrink-0">
          ...
          <button onClick={() => navigate(`/content/${selectedItem.id}`)}
