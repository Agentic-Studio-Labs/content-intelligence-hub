...
export default function Generated() {
  ...
  useEffect(() => {
    ...
    api.listGenerated(params).then((r) => { setItems(r.items); setTotal(r.total) })
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          <select value={formatFilter} onChange={(e) => setFormatFilter(e.target.value)}
            ...
            {['linkedin', 'email', 'twitter', 'summary'].map((f) => <option key={f} value={f}>{f}</option>)}
          </select>
          <select value={toneFilter} onChange={(e) => setToneFilter(e.target.value)}
            className="rounded-md border border-border bg-background px-2 py-1.5 text-sm">
            <option value="">All Tones</option>
            {['professional', 'casual', 'technical', 'friendly'].map((t) => <option key={t} value={t}>{t}</option>)}
          </select>
        </div>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} onClick={() => setSelectedItem(item)}
                  ...
                  <td className="px-3 py-2"><span className="text-xs px-2 py-0.5 rounded-full bg-muted">{item.format}</span></td>
                  <td className="px-3 py-2">{item.tone}</td>
                  <td className="px-3 py-2">{item.quality_score != null ? `${Math.round(item.quality_score * 100)}%` : '-'}</td>
                  <td className="px-3 py-2 text-muted-foreground">{new Date(item.created_at).toLocaleDateString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
          {items.length === 0 && <p className="text-center text-muted-foreground py-8 text-sm">No generated content yet. Repurpose content from the Library.</p>}
        </div>
        ...
        <div className="w-96 border border-border rounded-lg p-4 overflow-auto flex-shrink-0">
          <div className="flex justify-between items-start mb-3">
            <div>
              ...
              <button onClick={() => navigate(`/content/${selectedItem.source_content_id}`)}
                className="text-sm font-medium text-primary hover:underline">{selectedItem.source_title}</button>
            </div>
          ...
          <pre className="whitespace-pre-wrap text-sm font-sans leading-relaxed">{selectedItem.body}</pre>
        </div>
      )}
    </div>
  )
