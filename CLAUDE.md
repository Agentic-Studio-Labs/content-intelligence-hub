...
- **Terraform / GCP apply (Apr 2026):** **On `main`:** Resend magic-link mail (`cloud/shared/magic_link_mail.py`), Terraform **`resend_secret_id`** IAM + optional **`manage_cloud_tasks_queue`** / **`cloud_run_*_image`**, recovery docs in **`infra/gcp/README.md`**. A prior full apply created **Cloud SQL** `cih-cloud-sql`, GCS **`cih-artifacts-dev`**, empty **`anthropic-primary`** secret, and **Resend secretAccessor** IAM; it **may still need** fixing **Cloud Tasks** `409` (import **`google_cloud_tasks_queue.jobs[0]`** **or** **`manage_cloud_tasks_queue = false`**) and **Artifact Registry** images for **`cih-api`/`cih-worker`** before Cloud Run succeeds—see **`infra/gcp/README.md`** (“Partial apply / common errors”). **Merged:** PR **#10** (`feat/resend-magic-link-email` → `main`).
